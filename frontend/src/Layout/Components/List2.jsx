...
const List = ({ searchQuery, filter }) => {
  ...
  useEffect(() => {
    const fetchCustomers = async () => {
      try {
        const response = await axios.get("https://fitpreneurs.onrender.com/api/ptcustomer", {
          params: {
            search: searchQuery,
            filter: filter,
            page: page,
  ...
  const handleRemoveSession = async (id) => {
    ...
    try {
      await axios.put(`https://fitpreneurs.onrender.com/api/customer/${id}`,{sessionType: "0 Sessions"});
